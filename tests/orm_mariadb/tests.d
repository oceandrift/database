module tests.orm_mariadb.tests;
...
MariaDB getDB()
{
    auto db = new MariaDB("localhost", "u_oceandrift", "p_oceandrift", null);
    db.connect();
    dropRecreateUseDB(db);
    return db;
}

void dropRecreateUseDB(MariaDB driver)
{
...
unittest
{
    ...
    driver.execute(
        "CREATE TABLE `person` (id INTEGER PRIMARY KEY AUTO_INCREMENT, name TEXT, age INTEGER UNSIGNED)"
    );

    auto em = EntityManager!MariaDB(driver);
...
unittest
{
    ...
    auto em = EntityManager!MariaDB(db);
    ...
    {
        enum PreCollection!(Mountain, MariaDB) pcMt4000 =
            em.find!Mountain()
...

    {
        ...
        enum PreCollection!(Mountain, MariaDB) pc = em.find!Mountain()
                .where("height", '<', cap)
                .where("location", '=', loc);
...

    {
        ...
        enum PreCollection!(Mountain, MariaDB) pc = em.find!Mountain().where("location", '=', loc);
